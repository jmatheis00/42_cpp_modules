...
/*   Updated: 2023/09/29 23:36:59 by jmatheis         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void BitcoinExchange::getDatabase()
{
    ...
    while (getline(data_, buff))
    {
        if (buff != "date,exchange_rate")
        {
            ...
            if (delim == std::string::npos || buff.find('-') == std::string::npos || buff.find('-', buff.find('-') + 1) == std::string::npos || checkDate(buff.substr(0, buff.find(','))) == false || buff.substr(buff.find(',') + 1, buff.length()) == "" || checkFloatValue(buff.substr(buff.find(',') + 1, buff.length())) == -1)
            {
                std::cout << "data.csv: ";
                throw FormatError();
            }
...
{
    if (buff.find('-') == std::string::npos || buff.find('-', buff.find('-') + 1) == std::string::npos || buff.find('|', buff.find('-', buff.find('-') + 1) + 1) == std::string::npos)
        return (false);
    ...
    if (buff[numb] == '\0')
        return (false);
    if ((std::isdigit(buff[numb]) == false && buff[numb] != '-') || (buff[numb] == '-' && std::isdigit(buff[numb + 1]) == false))
        return (false);
    return (true);
}
...
bool BitcoinExchange::checkDate(std::string date)
{
    ...
    if (year > 1900 + timeunit->tm_year || year < 0 || month > 12 || month < 1 || day > 31 || day < 1 || (year == 1900 + timeunit->tm_year && month > 1 + timeunit->tm_mon) || (year == 1900 + timeunit->tm_year && month == 1 + timeunit->tm_mon && day > timeunit->tm_mday))
        return (false);
    if (((month == 4 || month == 6 || month == 9 || month == 11) && day > 30) || (month == 2 && day > 29) || (month == 2 && static_cast<int>(year) % 4 == 0 && static_cast<int>(year) % 100 != 0 && day > 28) || (month == 2 && static_cast<int>(year) % 400 == 0 && day > 28))
        return (false);
    return (true);
}
